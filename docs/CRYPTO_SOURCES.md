...
- **누락:** 현물 ETF 순유입과 채굴자 매도압은 미확보다. DefiLlama의 ETF 자료는 [유료 API 목록](https://api-docs.defillama.com/)에 있으나 이 수집기는 구매·구독하지 않는다. 별도 라이선스 자료나 검증 가능한 발행사 공시 연결 없이 값을 채우지 않는다.
